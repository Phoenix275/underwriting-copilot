...
const UWG_KB=[
 ...
 {k:['hand over','handover','vacation','out of office','cover for me','absence','reassign my queue','my backlog'],
...
function uwgWhatIfAnswer(q){
 ...
 if(line&&/raise|lower|move|change|set|if (i|we)/.test(q)){
  ...
  if(v>=10&&v<=95){
   ...
   return `Moving the approval line from <b>${A_LINE}</b> to <b>${v}</b>: auto-approvals go from <b>${now}</b> to <b>${then}</b> (<b>${d>=0?'+':''}${d}</b> cases, ${(then/CASES.length*100).toFixed(0)}% straight-through), and accepted cover would be about <b>${fmtBigMoney(newCov)}</b> against the ${fmtBigMoney(APPETITE_MONTHLY)} appetite. ${d>0?`Those ${d} case(s) stop getting a human look — the trade is throughput for scrutiny.`:'Fewer cases clear automatically; more underwriter time per policy.'}`;
  }
 }
