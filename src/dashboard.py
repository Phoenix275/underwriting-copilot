...
function uwgFieldAnswer(c,q){
 ...
 const F=[
  ...
  [/decision|status|verdict|outcome/, ()=>`<b>${c.decision}</b> — ${c.rate_class}`]];
 for(const [re,fn] of F)if(re.test(q))return `<b>${c.name}</b> (<span class="mono">${c.id}</span>): ${fn()}.`;
 return null;
}
function uwgFindByName(q){
 // match an applicant by name: every token of the name present in the query
 return CASES.find(c=>c.name.toLowerCase().split(/\s+/).every(t=>q.includes(t)))||null;
}
...
function uwgAnswer(qRaw){
 ...
 // applicant by name — field question or full case read
 const byName=uwgFindByName(q);
 if(byName){uwgLastCase=byName;return uwgFieldAnswer(byName,q)||uwgCaseAnswer(byName);}
 ...
 if(/(this case|current case|case here|explain (it|this)|why is (he|she|it))/.test(q)&&view==='case'&&activeId){
  const c=CASES.find(x=>x.id===activeId);if(c){uwgLastCase=c;return uwgFieldAnswer(c,q)||uwgCaseAnswer(c);}}
 // pronoun follow-up — "what is her case id" refers to the case we just discussed
 if(uwgLastCase&&/\b(he|she|his|her|hers|their|theirs|they|them|this person|that person|this applicant)\b/.test(q)){
  const f=uwgFieldAnswer(uwgLastCase,q);if(f)return f;
  if(/who|about|tell me/.test(q))return uwgCaseAnswer(uwgLastCase);}
